...
3. `docs/design/CODEX_PRIVACY_DASHBOARD_TASK.md`
4. `docs/design/harbor-ui-direction2-privacy-dashboard.svg`
5. the current live UI files referenced by the plan

The plan defines the target design. The review records semantic corrections found after comparing the mockup against Harbor's real architecture. The task file gives the implementation order and acceptance criteria. If the visual reference conflicts with the plan/review, **the plan/review wins**.
...
2. Refactor Personal dashboard using the scaffolded setup-state resolver and privacy facts.
...
5. Add local Settings, with Work-only DPC policy controls enforced by actual local profile-owner state.
...
8. Run the full repository verification and perform the final review checklist.
