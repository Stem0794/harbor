...
- Cross-profile file sharing is directional: Harbor clears `DISALLOW_SHARE_INTO_MANAGED_PROFILE` and installs parent-to-managed `ACTION_SEND`/`ACTION_SEND_MULTIPLE` filters. File selection occurs in Personal Harbor or another personal app; the work-profile Harbor share receiver is disabled in Personal, accepts only granted `content://` URIs in Harbor's owned work profile, and copies them into `Downloads/Harbor`. It never deletes the personal original or exposes a work-to-personal export target.
