...
- Harbor enables the supported personal-to-work file flow by clearing `DISALLOW_SHARE_INTO_MANAGED_PROFILE` and registering explicit `ACTION_SEND`/`ACTION_SEND_MULTIPLE` filters. Work-to-personal file sharing remains blocked by default. Existing profiles receive the policy when their work-profile Harbor instance starts; OEM file managers may still expose different move controls.
