...
- Harbor can clear the work-profile `DISALLOW_INSTALL_UNKNOWN_SOURCES` restriction so Android can show the normal per-source consent flow. When exactly one APK is sent through Harbor's Personal-to-Work flow, Work Harbor copies it to `Downloads/Harbor` and opens Android's package installer with a read-only content grant. Android still controls Harbor's per-source authorization, APK validation, permission display, and final confirmation; OEMs may block sideloading.
